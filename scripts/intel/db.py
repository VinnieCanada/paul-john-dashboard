...
# scripts/intel/db.py -> scripts/intel -> scripts -> workspace root
...
def log_collection(conn, source, status, reason=None, records=0):
    conn.execute(
...
def write_meetings(conn, meetings: list) -> int:
    records = 0
    now = _now_iso()
    for m in meetings:
...
def write_staff(conn, staff_list: list) -> int:
    now = _now_iso()
    count = 0
...
def search_meetings(conn, query: str, days: int = 30) -> list:
    rows = conn.execute(
...
def get_meeting_stats(conn) -> dict:
