...
Pulls meeting transcripts from Fireflies via GraphQL API.

Usage:
    python scripts/intel/collect_fireflies.py           # Last 7 days
    python scripts/intel/collect_fireflies.py --days 30
"""
...
# Load .env from workspace root (scripts/intel -> scripts -> workspace root)
...
def _build_transcript_text(sentences: list) -> str:
    lines = []
    for s in sentences:
...
def _build_participants(attendees: list) -> str:
    participants = []
    for a in attendees:
        p = {}
...
def collect(days: int = 7) -> list:
    ...
    while True:
        ...
        try:
            ...
            if status == 401:
                print("  Your API key may be invalid. Check app.fireflies.ai/integrations")
            return []
...
if __name__ == "__main__":
    ...
    if meetings:
        ...
        print(f"  Transcript: {len(meetings[0].get('transcript_text', ''))} chars")
    else:
